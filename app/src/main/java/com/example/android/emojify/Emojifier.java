    ...
    private static void whichEmoji(Face face){
        ...
        if(smiling) {
            if (leftEyeClosed && !rightEyeClosed) {
                emoji.setFace(Emoji.WINK_LEFT);
            }  else if(rightEyeClosed && !leftEyeClosed){
                emoji.setFace(Emoji.WINK_RIGHT);
            } else if (leftEyeClosed){
                emoji.setFace(Emoji.EYE_CLOSED_SMILING);
            } else {
                emoji.setFace(Emoji.SMILING);
            }
        } else {
            if (leftEyeClosed && !rightEyeClosed) {
                emoji.setFace(Emoji.FROWNING_WINK_LEFT);
            }  else if(rightEyeClosed && !leftEyeClosed){
                emoji.setFace(Emoji.FROWNING_WINK_RIGHT);
            } else if (leftEyeClosed){
                emoji.setFace(Emoji.EYE_CLOSED_FROWNING);
            } else {
                emoji.setFace(Emoji.FROWNING);
            }
        }

        // Log the chosen Emoji
        Log.d(LOG_TAG, "whichEmoji: " + emoji);
